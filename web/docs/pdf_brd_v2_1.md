...
| 角色 | 说明 | 与本系统的关系 | 系统角色 [V2.1] |
|------|------|----------------|----------------|
| 商家 | 上传 PDF 产品目录 | 数据生产者（不直接使用本系统） | — |
| 运营（上传员） | 上传 PDF、查看看板、管理标注员 | PDF 上传与进度跟踪 | `uploader` |
| 标注员 | 处理低置信度页面、人工标注 SKU | 主要用户（人机协同 + 全人工场景） | `annotator` |
| 下游系统 | 建品系统、搜索系统、以图搜商品 | JSON 消费者 | — |
| 研发/管理员 | 维护规则配置、阈值调优、管线迭代、用户管理 | 系统维护者 | `admin` |

> **[V2.1 新增] 系统认证说明**：系统实现 JWT 认证 + 基于角色的访问控制（RBAC），三种系统角色为 `admin`、`uploader`、`annotator`。用户通过登录页获取 JWT token，各 API 端点根据角色限制访问权限（如上传/创建 Job 需 uploader 角色，标注任务操作需 annotator 角色，用户管理需 admin 角色）。
