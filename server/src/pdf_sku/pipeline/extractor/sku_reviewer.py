...
4. color、specs 等字段是否来自页面上的实际印刷文字？如果是根据图片外观推断的（如看图片颜色填"米白色"），必须通过 corrected 清空该字段
...
class SKUReviewer:
    ...
    async def review(
        ...
        try:
            ...
            if pure_visual:
                prompt += ("\n\n重要提示: 此页面几乎没有文字标注。"
                           "所有 SKU 的 product_name、color、specs 等字段必须来自页面上实际印刷的文字。"
                           "如果某个 SKU 的名称是根据图片外观推断的描述（如\"餐椅\"、\"灰色沙发\"、\"木质餐桌\"），"
                           "而非页面上实际印刷的文字，应判定为 discard。")
            if scene_filter:
                prompt += ("\n注意: 此页面为场景展示页。"
