...
class SingleStageExtractor:
    ...
    async def detect_product_regions(
        ...
        """检测场景图中各个商品宣传区域的位置，返回归一化 bbox 列表。
        ...
        prompt = (
            "请识别这张产品图册页面中每个独立商品宣传区域的位置。\n\n"
            "要求:\n"
            "- 优先框选单个商品在页面中的完整宣传图/产品卡片区域，而不是只框商品主体轮廓\n"
            "- 以宣传图本身的边界为准，不要把 PDF 页面背景、页边白底、扫描留白一起框进去\n"
            "- 如果标题、型号、价格、规格、局部底板或留白本来就在同一张宣传图内部，应一起包含进来\n"
            "- 框选尽可能贴近宣传图内容边缘，不要留白边或多余背景边\n"
            "- 同一产品如果在页面上出现多张彼此分开的独立宣传图（如一张大图 + 一张小图/侧视图），每张都要分别框选并分别返回 bbox\n"
            "- 只有明显属于同一产品展示的小图才框选；装饰图、logo、材质纹理示意、场景干扰小图不要框选\n"
            "- 不要扩到其他无关产品，也不要把大面积公共背景、整页空白或跨产品场景一起框进去\n"
            "- 场景装饰物（灯、画、植物、地毯等）不要框选\n"
            "- 如果页面展示的是一组产品（如沙发组合），将整组对应的完整宣传区域作为一个区域框选\n"
