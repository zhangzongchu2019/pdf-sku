...
检测页面中的布局区域 (figure, table, text 等)，用于:
1. 合成大图拆分 (Phase 2c)
2. OCR 文字块空间聚类锚点 (Phase 2.6)
"""
...
_ALL_LABELS = {"figure", "picture", "image", "photo", "table", "text",
               "title", "list", "caption", "formula"}


@dataclass
class LayoutRegion:
    """布局检测区域。"""
    label: str          # figure | table | text | title | ...
...
def _remove_containing_boxes(
    ...
    if len(keep) < len(figures):
        logger.debug("layout_nms_removed", before=len(figures), after=len(keep))

    return keep


def detect_figures_on_image(image_data: bytes) -> list[tuple[float, float, float, float]]:
    """在图片上跑 YOLO 检测，返回 Figure 类 bbox 列表 (像素坐标)。"""
...
def detect_all_regions(image_data: bytes) -> list[LayoutRegion]:
    """检测图片上所有布局区域 (figure, table, text 等)。
    ...
    logger.debug("layout_detect_all", region_count=len(regions),
...
def split_composite_image(
    ...
    触发: 页面仅 1 张 search_eligible 图片，bbox 面积 > 60% 页面面积。
    """
