...
### V1.4 修订说明 (2026-02-27)

| 编号 | 变更 | 说明 |
|------|------|------|
| L1 | Phase 2c 合成大图布局检测 | 新增 `layout_detector.py`，引入 DocLayout-YOLO 检测全页大图中的 Figure 区域并拆分为独立子图 |
| L2 | 触发条件 | 页面仅 1 张 `search_eligible` 图片且 bbox 面积 > 60% 页面面积 |
| L3 | NMS 后处理 | 去除包含关系的冗余大框（overlap_ratio > 80% 的外框被移除） |
| L4 | Graceful degradation | `doclayout-yolo` 或模型文件不存在时静默跳过，不影响 pipeline 正常运行 |
| L5 | 坐标系对齐 | YOLO 像素坐标 → ÷ 缩放比 → PDF points；Phase 8 bbox × (150/72) → 截图像素坐标；`_crop_composites` 已支持 `_region_` 前缀 |
...
| **布局检测** | DocLayout-YOLO 合成大图 Figure 区域检测+拆分（Phase 2c） | V1.4:L1 |
